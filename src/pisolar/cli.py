...
def run(ctx: click.Context) -> None:
    ...
    if settings.renogy.enabled:
        ...
        scheduler.add_job(
            ...
            job_id="renogy_sensor",
        )
        logger.info("Renogy sensor enabled")
...
def check(ctx: click.Context) -> None:
    ...
    if settings.renogy.mac_address:
        renogy_sensor = RenogySensor(
            mac_address=settings.renogy.mac_address,
            device_alias=settings.renogy.device_alias,
        )
        try:
            readings = renogy_sensor.read()
            click.echo(f"  Renogy sensor: ✓ {len(readings)} reading(s)")
        except Exception as e:
            click.echo(f"  Renogy sensor: ✗ {e}")
    else:
        click.echo("  Renogy sensor: ✗ no MAC address configured")
...
def read_once(ctx: click.Context) -> None:
    ...
    if settings.temperature.enabled:
        ...
        for reading in readings:
            click.echo(
                f"  [temp] {reading.name}: {reading.value:.2f} {reading.unit}"
            )

    if settings.renogy.enabled and settings.renogy.mac_address:
        renogy_sensor = RenogySensor(
            mac_address=settings.renogy.mac_address,
            device_alias=settings.renogy.device_alias,
        )
        ...
        all_readings.extend(readings)
        for reading in readings:
            click.echo(f"  [solar] {reading.name}: {reading.to_dict()}")
