...
def listen_on_port(port, name):
    ...
    try:
        ser = serial.Serial(
            port=port, baudrate=9600, bytesize=8, parity="N", stopbits=1, timeout=0.1
        )
...
"""
    ...
    try:
        ...
        ser1 = serial.Serial(
            port=port1, baudrate=9600, bytesize=8, parity="N", stopbits=1, timeout=1
        )

        # Open port 2 for receiving
        ser2 = serial.Serial(
            port=port2, baudrate=9600, bytesize=8, parity="N", stopbits=1, timeout=1
        )
