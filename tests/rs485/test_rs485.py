...
def test_rs485_connection(
    port="/dev/ttyUSB0", baudrate=9600, slave_address=1, verbose=True
):
    ...
    client = ModbusSerialClient(
        port=port, baudrate=baudrate, bytesize=8, parity="N", stopbits=1, timeout=3
    )
    ...
    for reg_addr, name, scale, unit in test_registers:
        try:
            ...
            if result.isError():
                ...
                print(
                    f"  ✓ Register 0x{reg_addr:04X} ({name}): {scaled_value:.2f} {unit} (raw: {raw_value})"
                )
                success_count += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test RS485 connection to Renogy Rover Elite"
    )
